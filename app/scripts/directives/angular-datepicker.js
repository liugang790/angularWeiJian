      ...
      return [
        '<div class="_720kb-datepicker-calendar-header">',
          '<div class="_720kb-datepicker-calendar-header-left">',
            '<b href="javascript:void(0)" ng-click="prevMonth()" title="{{ buttonPrevTitle }}">',
              prevButton,
            '</b>',
          '</div>',
          ...
          '<b href="javascript:void(0)" ng-click="nextMonth()" title="{{ buttonNextTitle }}">',
            nextButton,
          '</b>',
          '</div>',
        '</div>'
    ...
    , generateDays = function generateDays() {
      ...
      return [
        '<div class="_720kb-datepicker-calendar-body">',
          '<a href="javascript:void(0)" ng-repeat="px in prevMonthDays" class="_720kb-datepicker-calendar-day">',
            '{{px}}',
          '</a>',
          '<a href="javascript:void(0)" ng-repeat="item in days" ng-click="setDatepickerDay(item)" ng-class="{\'_720kb-datepicker-active\': day === item, \'_720kb-datepicker-disabled\': !isPriceDay(year, monthNumber, item)}" class="_720kb-datepicker-calendar-day">',
            '{{item}}',
            '<span  class="price">{{priceOnDay(item)}}</span>',
          '</a>',
          '<a href="javascript:void(0)" ng-repeat="nx in nextMonthDays" class="_720kb-datepicker-calendar-day">',
            '{{nx}}',
          '</a>',
        '</div>'
      ];
    ...
    , datepickerDirective = function datepickerDirective($window, $compile, $locale, $filter, $interpolate) {
      ...
      var linkingFunction = function linkingFunction($scope, element, attr) {
        var selector = attr.selector
          ...
          , theCalendar
          , defaultPrevButton = '<i class="iconfont left">&#xe628;</i>'
          , defaultNextButton = '<i class="iconfont right">&#xe600;</i>'
          ...
          , setInputValue = function setInputValue() {
            ...
            if ($scope.isSelectableMinDate($scope.year + '/' + $scope.monthNumber + '/' + $scope.day) &&
              ...
              // }
              // console.log("picked:"+$scope.pickedDate);

              if ($scope.day<10) {
                $scope.day = '0' + $scope.day;
              };

              $scope.pickedDate = $scope.year + '-' + $scope.monthNumber + '-' + $scope.day;

              $scope.$parent.changePrice($scope.pickedDate);
        ...
        $scope.nextMonth = function nextMonth() {
          ...
          if ((currentDate.getFullYear()*12+currentDate.getMonth()+2) < ($scope.monthNumber+$scope.year*12)) {
            return;
          };
        ...
        $scope.prevMonth = function managePrevMonth() {
          var currentDate = new Date();
          if ((currentDate.getFullYear()*12+currentDate.getMonth()) >= ($scope.monthNumber+$scope.year*12 - 1)) {
            return;
          };
        ...
        $scope.setDatepickerDay = function setDatepickerDay(day) {
          if ($scope.isPriceDay($scope.year, $scope.monthNumber, day) == false) {
            return;
          };
        ...
        $scope.priceOnDay = function priceOnDay(day){
          if (day < 10) {
            day = "0"+day;
          };
          var dateString = $scope.year + "-" +$scope.monthNumber + "-" + day;
          if ($scope.comboWithPrice[dateString]) {
            return "￥"+$scope.comboWithPrice[dateString][1];
          }
        }

        $scope.isPriceDay = function isPriceDay(year, month, day){
          if (day < 10) {
            day = "0"+day;
          };
        ...
        $scope.isDiscountDay = function isDiscountDay(monthNumber, year, day){
          if ($scope.discount_days) {
            for (var i = $scope.discount_days - 1; i >= 0; i--) {
                date_arrays = discount_day.split(":");
                if (discount_day[0]==year && discount_day[1] == monthNumber && discount_day[2] == day) {
                  return true;
                }
            };
            return false;
        ...
        $scope.daysInString = ['日', '一', '二', '三', '四', '五', '六']
        ...
        thisInput.append($compile(angular.element(htmlTemplate))($scope));
        //get the calendar as element
        $compile(angular.element(htmlTemplate))($scope)
